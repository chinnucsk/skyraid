...
account_info(#skr_auth_acctoken{provider=dropbox,
				token=[{"oauth_token_secret",
					TokenSecret},
				       {"oauth_token", Token},
				       {"uid", _Uid}]}=AT) ->
...
list_files(#skr_auth_acctoken{provider=dropbox,
...
write_file(#skr_auth_acctoken{provider=dropbox,
	   ...
	   FileName,
	   Content) ->
    file_put(?key, ?secret, Token, TokenSecret,
	     "dropbox", FileName, Content).

read_file(#skr_auth_acctoken{provider=dropbox,
			     token=[{"oauth_token_secret", TokenSecret},
				    {"oauth_token", Token},
				    {"uid", _Uid}]}, FileName) ->
...
to_account({Account}, AccessToken) ->
    build_account(Account,
		  #skr_account{storage_id=dropbox,
...
build_account([{<<"uid">>, Value} | Rest], A) ->
    build_account(Rest, A#skr_account{id={dropbox, Value},
				      ext_id=Value});
build_account([{<<"country">>, Value} | Rest], A) ->
    build_account(Rest, A#skr_account{country=Value});
build_account([{<<"quota_info">>, {[{<<"shared">>,Shared},
				    {<<"quota">>,Quota},
				    {<<"normal">>,Normal}]}} | Rest],
	      A) ->
    build_account(Rest,
		  A#skr_account{quota_info=#skr_quota_info{
					      shared=Shared,
					      quota=Quota,
					      normal=Normal}});
...
request_token(Key, Secret) ->
    {ok, RequestToken} =
	oauth:post("https://api.dropbox.com/1/oauth/request_token",
		   [],
...
access_token(Key, Secret, Token, TokenSecret) ->
    {ok, AccessToken} =
	oauth:post("https://api.dropbox.com/1/oauth/access_token",
		   [],
		   {Key, Secret, hmac_sha1},
		   Token,
		   TokenSecret),
...
account_info(Key, Secret, Token, TokenSecret) ->
    {ok, {_, _, AccountInfo}} =
	oauth:get("https://api.dropbox.com/1/account/info",
		  [],
		  {Key, Secret, hmac_sha1},
		  Token,
		  TokenSecret),
...
file_get(Key, Secret, Token, TokenSecret, Root, Path) ->
    {ok, {_, _, File}} =
	oauth:get("https://api-content.dropbox.com/1/files/" ++
		      Root ++ "/" ++ Path,
		  [],
		  {Key, Secret, hmac_sha1},
		  Token,
...
file_put(Key, Secret, Token, TokenSecret, Root, Path, Content) ->
    {ok, {_, _, File}} =
	oauth:put("https://api-content.dropbox.com/1/files_put/" ++
		      Root ++ "/" ++ Path,
		  [],
...
metadata(Key, Secret, Token, TokenSecret, Root, Path) ->
    {ok, {_, _, Metadata}} =
	oauth:get("https://api.dropbox.com/1/metadata/" ++
		      Root ++ "/" ++ Path,
		  [],
		  {Key, Secret, hmac_sha1},
		  Token,
...
to_account_test() ->
    ...
    #skr_account{display_name = <<"Apa Nilsson">>} =
	to_account(A, #skr_account{storage_id=dropbox}).
