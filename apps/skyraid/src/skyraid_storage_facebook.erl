...
access_token(#skr_auth_reqtoken{provider=?provider,
...
account_info(#skr_auth_acctoken{provider=?provider,
				token=[{"oauth_token_secret",
					TokenSecret},
				       {"oauth_token", Token},
				       {"uid", _Uid}]}=AT) ->
...
oauth_get(URL, Params, Consumer, Token, TokenSecret) ->
    Signed =
	oauth:sign("GET", URL, Params, Consumer, Token, TokenSecret),
    {AuthorizationParams, QueryParams} =
	lists:partition(fun({K, _}) ->
				lists:prefix("oauth_", K) end, Signed),
    Request = {oauth:uri(URL,
			 QueryParams),
	       [oauth:header(AuthorizationParams)]},
    httpc:request(get, Request, [{autoredirect, false}], []).

request_token(Key, Secret) ->
    oauth_get("https://www.google.com/accounts/OAuthGetRequestToken",
	      [{"scope", "http://www.google.com/calendar/feeds/"}],
	      {Key, Secret, hmac_sha1}, "", "").

access_token(Key, Secret, Token, TokenSecret) ->
    {ok, AccessToken} =
	oauth:post("https://www.google.com/accounts/OAuthGetAccessToken",
		   [],
		   {Key, Secret, hmac_sha1},
		   Token,
		   TokenSecret),
...
account_info(Key, Secret, Token, TokenSecret) ->
    {ok, {_, _, AccountInfo}} =
	oauth:get("https://www.googleapis.com/drive/v2/about",
		  [],
		  {Key, Secret, hmac_sha1},
		  Token,
		  TokenSecret),
